...
int flexForm(WINDOW *win, DBnode_t *db, const char *formName)
{
    ...
    do { // Loop through to get user requests to pass at form_driver function
        ...
        case KEY_IC: // toggle insert char mode
            ...
            mvwprintw(my_form_win, 1, getmaxx(my_form_win) - 6, ((insertMode == REQ_INS_MODE) ? "OVL" : "INS")); // print the insert mode in win_body
            break;
    ...
    if (store) { // if input data was confirmed by user
        ...
        sdstrim(ptrDB->fname, " ");
        ptrDB->lname = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->lname, " ");
        ptrDB->organization = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->organization, " ");
        ptrDB->job = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->job, " ");
        ptrDB->hphone = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->hphone, " ");
        ptrDB->wphone = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->wphone, " ");
        ptrDB->pmobile = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->pmobile, " ");
        ptrDB->bmobile = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->bmobile, " ");
        ptrDB->pemail = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->pemail, " ");
        ptrDB->bemail = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->bemail, " ");
        ptrDB->address = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->address, " ");
        ptrDB->zip = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->zip, " ");
        ptrDB->city = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->city, " ");
        ptrDB->state = sdsnew(field_buffer(field[i++], 0));
        sdstrim(ptrDB->state, " ");
        ptrDB->country = sdsnew(field_buffer(field[i++], 0));
...
int set_field_digit(FIELD *field, int buf, int digit)
{ // the equivalent of set_field_buffer for INTEGER field
    sds string;
    int retVal;
...
int field_digit(FIELD *field, int buf)
{                                          // the equivalent of field_buffer for INTEGER field
